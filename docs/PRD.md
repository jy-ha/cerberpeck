...
설치와 실험은 안전한 기본값을 채운 뒤 별도 확인을 기다리지 않고 계속한다. 진행 화면은 관찰과 중단을 위한 것이며 매 단계 승인을 받는 마법사가 아니다. 인증·비밀정보 누락, 프로덕션 또는 외부 시스템 변경, 복구할 수 없는 충돌처럼 자동 진행이 위험한 경우에만 멈춘다.
...
cerberpeck install                # 기본값으로 즉시 설치, TUI로 진행 표시
cerberpeck install --interactive  # 선택을 바꾸는 설치 TUI
...
`cerberpeck run`은 TTY에서 확인 입력이 필요 없는 진행 TUI를 표시한다. 상단에는 목표와 현재 Champion, 본문에는 라운드·Action·Agent 세션 상태, 하단에는 경과 시간·누적 사용량·현재 단계가 보인다. 기본 키는 `p` 일시정지, `r` 재개, `q` 안전한 중단, `d` 상세 로그 전환뿐이다. 키를 누르지 않아도 완료까지 계속된다.

비TTY에서는 같은 event를 한 줄씩 출력한다. `--json`에서는 stdout을 JSONL event 전용으로 사용하고 사람용 로그는 stderr로 보낸다.
...
2. 최신 또는 지정 버전 release manifest 다운로드
3. 임시 디렉터리에 installer binary 다운로드
4. SHA-256 checksum 검증
5. 지원될 경우 release signature 검증
6. TTY가 있으면 진행 TUI, 없으면 line-mode 연결
7. installer binary 실행 후 임시 파일 정리
...
`cerberpeck install`과 원라인 설치는 안전한 기본값을 즉시 적용하면서 다음 진행 화면만 보여준다.

```text
Cerberpeck 0.1.0 · workspace · Codex + Claude
✓ 대상 검사
✓ CLI 설치
● Skill 설치
○ Browser 준비
...
기본 선택은 Workspace, 감지된 모든 호스트와 감지된 system browser다. portable v0.1에서 system browser가 없으면 `none`으로 설치하고 doctor가 capture 비활성화를 알린다. managed Chromium 다운로드는 P1에서 제공한다. 설치 계획과 변경 경로는 화면에 보이지만 확인 입력을 기다리지 않는다.

사용자가 `--interactive`를 명시한 경우에만 한 화면에서 Workspace 경로, Scope, Hosts와 Browser를 바꿀 수 있다. Enter 즉시 설치하며 별도의 Confirm 화면은 두지 않는다. 이미 Cerberpeck이 소유한 수정 파일은 자동 백업 후 교체하고, 다른 소유자의 파일과 충돌할 때만 멈춘다.
...
- TTY 여부와 관계없이 생략된 값은 같은 안전한 기본값으로 추론하고 확인 없이 설치한다.
- `--yes`는 기존 script 호환을 위해 허용하지만 기본 자동 진행과 동작이 같다.
...
```text
cerberpeck update
cerberpeck update --version 0.2.0
cerberpeck update --check
```
...
- TTY에서는 입력 없이 완료까지 갱신되는 진행 TUI를 표시하고 비TTY에서는 line event를 출력한다.
...
- macOS·Linux에서 `curl | sh` 한 줄로 Workspace 기본 설치를 입력 없이 완료하고 진행 TUI 또는 line-mode를 표시한다.
...
- scope, workspace, hosts, browser와 version을 flag로 지정할 수 있다.
...
- 입력 없는 Workspace 기본 설치와 직접 구현 설치·진행 TUI
...
- 기본 설치와 실험은 확인을 기다리지 않고 끝까지 진행하며 TUI는 관찰·중단 수단으로 사용한다.
